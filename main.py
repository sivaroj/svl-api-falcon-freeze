        ...
        if (username is None) or (password is None):
            ...
            if ('dn_no' in params):
                ...
                if ('emp_no' in params):
                    ...
                    response(resp, 'GET, OPTIONS', falcon.HTTP_404, 'error: Require Emp_no.')
                data = RouteForTimestampDn.get_data(self, user=username, password=password, dn_no=dn_no, emp_no=emp_no)
                response(resp, 'GET, OPTIONS', falcon.HTTP_200, data)
            else:
                response(resp, 'GET, OPTIONS', falcon.HTTP_404, 'error: Require DN.')
    ...
    def on_get(self, req, resp):
        ...
        if (username is None) or (password is None):
            ...
            for key, value in req.params.items():
                ...
                data = GetEmpFromIdCard.get_data(self, user=username,password=password, id_card=id_card, emp_no=emp_no)
                response(resp, 'GET, OPTIONS', falcon.HTTP_200, data)
